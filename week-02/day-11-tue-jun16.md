# Tuesday, 16 June 2026 · Day 11

**Week 2 · Phase 1 · Foundation**

---

## 🔴 9:30 – 10:30 am · DSA

**Problem:** [Product Except Self](https://leetcode.com/problems/product-of-array-except-self)
**Pattern:** Arrays & Hashmaps

### Steps
1. Open LeetCode: `leetcode.com/problems/product-of-array-except-self`
2. Set a **25 min timer** — attempt alone, no hints
3. After timer: watch NeetCode explanation → `youtu.be/bNvIQI2wAjk`
4. Write your approach in 2–3 lines below

### My Solution Approach
```
(write here after solving)
```

### What I learned
```
(write after watching the explanation)
```

### ✅ Done? Check off in [progress.md](../progress.md)

---

## ⚫ 1:30 – 2:00 pm · Lunch
**Free — eat with colleagues. No study.**

---

*← [Back to Week 2](./README.md)*
